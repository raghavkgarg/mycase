...
### 2. Created Intraday Performance Command-line Tool
- Created the CLI utility at [main.go](file:///Users/raghavgarg/Projects/myGo/mycase/cmd/performance/main.go).
- Supported CLI flags:
...
`go run cmd/performance/main.go -file data/mymicro.csv -capital 100000 -date 2026-07-10 -time 09:30`
...
* **The Fix**: 
  * Aligned the mock data generator in [main.go](file:///Users/raghavgarg/Projects/myGo/mycase/cmd/monitoring/main.go) to count **backwards from today**, ensuring all mock histories extend to the current date.
...
* **The Update**: We updated the automatic pipeline runner in [main.go](file:///Users/raghavgarg/Projects/myGo/mycase/cmd/pipeline/main.go) to prompt the user before running Step 8:
  ```text
  ...
  ```
  * Choosing option 2 forwards the `-date` parameter to `./bin/monitoring`, making the simulation run on the exact same window as Step 7.
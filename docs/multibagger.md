...
| Code | Safety Constraint / Filter | New Candidate Entry Limit | Existing Holding Exit Cushion (`isExisting`) | Operation & Robust Fallbacks |
| :--- | :--- | :--- | :--- | :--- |
| **A** | **Market Cap Range** | ₹500 Cr - ₹50,000 Cr | Same | Absolute size limits to balance growth headroom & liquidity. |
| **B** | **Average Daily Volume (ADV)** | $\ge$ ₹1 Crore | Same | Average Volume $\times$ Regular Price. Ensures exit liquidity. |
| **C** | **Quality of Earnings (CFO/PAT)** | $\ge$ **25%** | $\ge$ **20%** (20% soft cushion) | Verifies profits are backed by cash. CFO must be > 0. |
| **D** | **Consistent Earnings Growth** | **Disabled (`false`)** | Disabled | Bypassed in config to allow turnaround / cyclical recovery stories. |
| **E** | **Promoter Shareholding** | $\ge$ **25%** | $\ge$ **22.5%** (10% soft cushion) | Ensures promoter skin-in-the-game. |
| **F** | **Trend & Buffer Guard (200-SMA)** | Ratio $\ge$ **0.95** | Ratio $\ge$ **0.90** (10% dip cushion) | Allows max 5% dip for new entries, 10% for existing holdings. Layer 2 slope check strictly evicts if 200-SMA is trending downward (>0.5% drop over 20d). |
| **G** | **Promoter Pledging** | < 5% | < 5% | Prevents exposure to margin call liquidations. |
...
### F. Downtrend & Buffer Guard (200-Day SMA Check)
* **Rule:** Enforces a **Dual-Layer Trend Safety Filter with Asymmetric Entry/Exit Cushions**:
  1. **SMA Buffer Floor (Layer 1)**:
     - **New Candidate Entry Limit (`0.95`)**: Latest price must be $\ge$ **95%** of the 200-day Simple Moving Average ($Price / SMA_{200} \ge 0.95$). Prevents initiating new positions in lagging or broken stocks.
     - **Existing Holding Soft Cushion (`0.90`)**: Existing portfolio holdings are granted a soft cushion down to **90%** of the 200-SMA ($Price / SMA_{200} \ge 0.90$). Prevents binary cliff panic-liquidations on standard 5–10% market drawdowns.
  2. **Slope Guard (Layer 2 Downward Trajectory Check)**: If the price is below the 200-SMA ($Price < SMA_{200}$), the system evaluates the **20-day slope of the 200-SMA line itself**. If the 200-SMA line has declined by $>0.5\%$ over the preceding 20 trading days, the stock is confirmed to be in a structural downward bear trajectory and is evicted regardless of cushion.
* **Why:** Eliminates binary cliff liquidations (e.g. exiting at 0.94 on a rising 200-SMA) while strictly protecting the portfolio against genuine structural trend breakdowns.
...
```text
       ...
       New Entrant: Price/200-SMA >= 0.95                   Is 200-SMA Line Sloping Down?
       Existing:    Price/200-SMA >= 0.90                   (Evaluated over 20 trading days)
                         │                                                   │
          ┌──────────────┴──────────────┐                     ┌──────────────┴──────────────┐
          ▼                             ▼                     ▼                             ▼
       FAIL (< Floor)               PASS (>= Floor)         SLOPING DOWN (>0.5% drop)   FLAT OR UPTREND
       ❌ Evicted: Severe Dip      Proceed to Slope Check   ❌ Evicted: Bear Trend       ✅ Retained: Normal Pullback
```
...
| **III. Institutional Thrust** (30 pts) | **Breakout Volume Intensity** | 15 pts | Rank by the volume multiplier of their breakout day. A 6.0x volume spike scores higher than baseline 2.0x. Deliverable Volume % (`f.DeliveryPct`) from NSE `nselib` scales the multiplier (`1.0 + (DeliveryPct/100)*0.5`) to reward institutional delivery-based accumulation. |
...
2. **Data Acquisition**:
   ...
   - Fundamental sheets are fetched concurrently (`pkg/yfinance/yfinance.go`), enriched with official earnings/board meeting dates and deliverable volume percentages (`DeliveryPct`) fetched in batch via Python `nselib` (`scripts/fetch_nse_data.py`). Cached in DuckDB (`pkg/cache`).
3. **Pre-Filtering (Safety Filters with Asymmetric Exit Cushions)**: 
   Candidates are evaluated in `pkg/stockpicker/filters.go`'s `isEligible` function. Any ticker that fails the 11 absolute metrics (Market Cap, ADV, Cash Flow, Earnings Trend, Promoter Share %, 200-SMA, Pledge %, ROCE, Debt/Equity, Interest Coverage, CROIC) is eliminated. If `multibagger` is chosen, they must also pass the 4 advanced checkpoints (Sales Growth Accelerator, Operating Leverage CapEx Inflection, DSO Improvement, and volume breakouts). Active portfolio holdings (`isExisting = true`) receive soft-band tolerance cushions (e.g. 0.90 200-SMA floor, 1/3 operational criteria, relaxed ROCE/CROIC/leverage) to eliminate binary cliff turnover.
...
* **Operational Metrics Check (Asymmetric Entry vs. Exit Gate)**:
  ...
  - **Existing Holding Retention Gate ($\ge 1/3$)**: Existing active holdings must satisfy at least **1 out of the 3** operational conditions. This eliminates binary cliff evictions on temporary quarterly growth normalization while allowing score and rank hysteresis to naturally govern their standing.
...
## 8. Portfolio Lifecycle & Anti-Churn Architecture
...
### Pillar 1: Re-Entry Anti-Churn Cooldown Window
* **Rule**: When an active portfolio holding is exited (liquidated to `0.00%` weight), it enters an automatic **30-Day Cooldown Window** (`cooldown_days: 30`).
...
1. **200-Day SMA Ratio**:
   - **New Entrants**: $Price / SMA_{200} \ge 0.95$ (Never initiate positions in lagging momentum).
   - **Existing Holdings**: $Price / SMA_{200} \ge 0.90$ (Allows 10% pullback from 200-SMA without panic selling).
   - **Layer 2 Slope Check**: Evaluates if the 200-SMA line has fallen $>0.5\%$ over 20 trading days. Structural downtrends are evicted immediately regardless of cushion.
2. **Operational Acceleration**:
   - **New Entrants**: Must meet $\ge 2 / 3$ acceleration criteria (Sales acceleration, Asset turnover expansion, DSO improvement).
   - **Existing Holdings**: Must meet $\ge 1 / 3$ operational criteria.
3. **Fundamental Quality Ratios**:
   - **ROCE**: Floor relaxes from 12.0% to 10.2% (15% buffer).
   - **CROIC**: Floor relaxes from 6.0% to 4.8% (20% buffer).
   - **Debt/Equity**: Cap relaxes from 1.5 to 1.725 (15% buffer).
   - **Interest Coverage**: Floor relaxes from 3.0 to 2.7 (10% buffer).
...
### Pillar 4: Exact Funnel Conservation & Audit Reporting
* Every rebalance run validates mathematical constituent conservation via `SelectionFunnel.Validate()`:
  $$\text{Stage-1 Survivors} = \text{RegimeRejected} + \text{SectorCapped} + \text{CooldownBlocked} + \text{RankLimited} + \text{FinalSelected}$$
...
During the system-wide data integrity audit (see `docs/earlyMB.md` §26, "System-Wide Data Integrity Audit"), a critical vulnerability was identified in how fundamental data was ingested for the multibagger strategy:

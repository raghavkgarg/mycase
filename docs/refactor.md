...
|-------|------|--------|-----------|
| Loose ends | Wire-or-delete L1–L8 (~~render~~✅, ~~selections~~✅, ~~config_json~~✅, ~~final stage~~✅→P9, server paths, ~~tax effect~~✅, ~~EmailAlerter~~✅, ~~skip-masking test~~✅) | 🟢 L1–L4+L6+L7+L8 done (L4 built as roadmap P9); only L5 remains (deferred to R15) | none |
...
| L2 | ✅ **CLOSED (delete → later built as Phase 8).** The `selections` DuckDB table + `Selection` struct + `Insert/Get/GetPreviousSelections` were built (Phase 7) but the write path was never wired — nothing called `InsertSelections`, so `pipeline show`'s "Final Selections" section always rendered empty. Wiring it is real feature work, so it was **deleted** here rather than left dead; the intended richer feature was captured as **roadmap Phase 8**, which has since been **implemented** (scaffolding recovered from this commit, write path wired in `pkg/autopilot`, structured driver metrics added to `selectiontracker`/`stockpicker.PickResult`, `SaveReport` migrated off text-parsing, "Final Selections" restored in `pipeline show`). | was: `cmd/pipeline.go` writes no selections; only `*_test.go` calls `InsertSelections` | Done (deleted here; feature built in Phase 8). |
...
| L4 | ✅ **CLOSED (built as Phase 9).** The proposal `"final"` stage was *defined but unimplemented* — `pipeline show`/`diff` referenced it but nothing wrote it. Roadmap **Phase 9** built the write path: `autopilot.FinalStageProposals` converts a confirmed proposal's successfully-placed BUY orders into `final`-stage proposals (weighted by executed value), and `pkg/server` `handleAutopilotConfirm` persists them (best-effort, `slog`-logged) via `cache.LatestRun` + `InsertProposals(runID, "final", …)`. `attribution.LoadRebalanceHistory` now prefers `final` over `optimized`. Intent-based (submitted weights) at confirm time, plus a realized-fill reconcile (`mycase pipeline reconcile <run_id>`) that overwrites `final` with actual executed weights from broker transactions (`autopilot.RealizedStageProposals` + `cache.DeleteProposalsStage`). | was: `GetProposals(runID, "final")` returned nothing | Done (Phase 9). |
...
| L6 | ✅ **CLOSED (drop).** The tax line is removed from the *live* decomposition paths (CLI `mycase performance --decompose` + dashboard Performance tab): the CLI no longer renders a "Tax effect" row and now prints a pointer to `mycase tax`; the dashboard drops the `tax` JSON field + row and points to the Tax tab. Rationale: TLH impact is a cash effect outside the `Selection + Rebalancing = Active` price-return identity, and a rate-weighted realized-loss estimate would *overstate* the benefit (ignores the basis reset on the wash-sale-avoiding repurchase). Tax stays surfaced in full ST/LT detail by `mycase tax` / the Tax tab. The `Decomposition.Tax` field + `DecomposeInput.TaxSaving` remain in the reusable API (tested) for callers that supply their own figure. | was: `cmd/performance.go`, `performance_handler.go` passed `TaxSaving: 0` | Done. |
...
- **`themereturn` + `kiteauth` still use `fmt.Print*` for diagnostics**, not `slog` (violates the logging steering two-channel rule). They came from `main`, which predates R14/R18. Acceptable for now because both are *dormant* (not on the live US path); migrate to `slog.*Context` if either is ever promoted to a live flow. Same class of debt R18 closed for `stockpicker`/`executor`.
- **`pkg/alert/email.go` deliberately NOT resurrected.** It was a stub (`"email alerts not yet implemented"`), unreferenced on `main`, pruned on the EBM branch in `da6b43b` as dead-stub cleanup. `main` never developed it, so it is genuinely dead code, not a dormant *feature* — resurrecting it would contradict the cleanup for zero benefit. If SMTP alerting is ever wanted, implement it fresh against the current `alert.Alerter` interface.
- **Advisory `unreachable func` findings are expected, not bugs.** The non-cooldown selection wrappers (`SelectTopNValue/Multibagger/Standard/USQM`), `themereturn.RenderBanner`, `kiteauth` entry points, etc. show as unreachable because they are dormant or thin backward-compat delegators. This is the intended state of the dormant-code doctrine; do not "clean them up" without a deliberate decision to drop the feature.
- **`betteralign` struct-field-ordering suggestions on resurrected structs** (`themereturn/*`, `kiteauth/autologin.go`, `stockpicker/cooldown.go`, `config` structs that gained cooldown fields) are unaddressed. Advisory only (GC-scan micro-optimization); apply with `betteralign -apply ./...` in a dedicated cosmetic pass if desired.

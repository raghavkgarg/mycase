...
| **Tests (partial)** + **R2.4 partial** | `CapWeights` extracted from `cmd/optimize.go` → `pkg/optimizer/cap_weights.go` (exported). 28 new tests across `pkg/optimizer` (capWeights + math edge cases + quick-check property test), `pkg/monitoring` (simulator determinism, empty portfolio, insufficient history, NaN guard, boundary cases), `pkg/config` (MFSConfig, Themes, Config round-trip — all graceful fallback paths). | `6a74e2d` |
| **Cleanup** | `make cleanup` now passes clean. Fixed: ST1005 error-string punctuation (`cmd/merge.go`), SA1019 deprecated `strings.Title` → inline `ToUpper` (`cmd/monitor.go`, `cmd/report.go`), S1039 unnecessary `fmt.Sprintf` (`pkg/selectiontracker/tracker.go`), SA5011 nil-pointer guard on `hardFilters` (`pkg/stockpicker/scoring.go`). Go toolchain bumped 1.26.3 → 1.26.5 (3 stdlib CVEs resolved: GO-2026-5856, GO-2026-5039, GO-2026-5037). Full `gofmt`+`go fix` pass across all 32 source files. | `ef20714` |
| **Tests** | Coverage baseline established. 35 → 96 top-level passing tests (110 incl. subtests). New test files: `pkg/yfinance/metrics_test.go` (RSI bounds/extremes, SalesGrowth, DSO, VolumeBreakout), `pkg/yfinance/prices_test.go` (MapTickerToYahoo, CleanIntradayNoise), `pkg/optimizer/math_test.go` (CalculateMean, Covariance, DownsideDeviation, TotalReturn edge cases), `cmd/cmd_test.go` (parsePerfDate, cleanBasketArg, PipelineConfig.UnmarshalYAML). Extended: `pkg/csvloader/loader_test.go` (ParseBasket validity/header/duplicate/invalid; GetUniverseName quick-check property), `pkg/stockpicker/stockpicker_test.go` (normalizeValue out-of-range; ApplyHysteresisSelection; ApplyRebalancingBand). Two production fixes found by tests: (1) `GetUniverseName("")` returned `""` — skip empty parts from split; (2) `normalizeValue` extrapolated negative scores for out-of-range vals — clamp to [0, maxPoints]. `go test -race ./...` passes clean. | `8dc4c18` |
...
| **R6** | `pkg/costs/costs.go`: `CostModel`, `CostBreakdown`, `Calculate` for Indian equity delivery (STT, stamp duty, DP ₹15.93/ISIN/sell, SEBI). `pkg/costs/tax.go`: `ClassifySell` with Finance Act 2024 rates (STCG 20%, LTCG 12.5% above ₹1.25L); handles zero purchase date (not available from broker API). `pkg/optimizer/rebalance.go`: `FilterMicroTransactions` drops orders where cost/value > threshold. `cmd/basket.go`: micro-tx filter + STCG/LTCG warning banner wired in before execution. | `454e2a8` |
| **R7** | `pkg/backtest/`: `types.go` (SimConfig, Holding, DailySnapshot, SimResult), `engine.go` (date-aligned simulation, sell-then-buy rebalance with slippage), `metrics.go` (CAGR, MaxDrawdown, Sharpe, Sortino, Calmar, Beta, Alpha at 6% India risk-free). `pkg/yfinance`: `FetchHistoricalByDateRange` (period1/period2 API). `pkg/cache`: `GetPricesByDateRange`/`StorePricesByDateRange` (historical ranges never expire). `cmd/backtest.go`: concurrent price fetch, yearly breakdown table. | |
...
|-------|------|--------|----------|--------|
| **Tests** | Coverage baseline before R2 changes logic | Medium (2d) | P1 | ✅ `8dc4c18` |

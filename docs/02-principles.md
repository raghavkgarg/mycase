...
- **Cache-first, every provider.** `pkg/cache/prices.go` `GetPrices` returns a miss unless `isFreshToday` (the ticker's own market calendar via `marketcal.ClockForTicker` — NYSE for `US:`, NSE otherwise); `GetPricesByDateRange` treats historical ranges (ending before today) as never-expiring. Fundamentals use a rolling 24h / market-EOD TTL (`isFreshFundamentals`, `pkg/cache/fundamentals.go`). **Both providers check the cache before the network**: the Yahoo path via `pkg/yfinance/duckdbcache.go`, and the Schwab path via `pkg/broker/schwab/duckdbcache.go` (prices) plus `pkg/datafetcher/fundcache.go` (fundamentals). Rows are keyed by the full prefixed ticker and tagged with a `source` provenance column (`"yahoo"`, `"schwab"`, `"schwab+edgar"`). One subtlety: US **fundamentals** are cached in the **Router** (L3), not the Schwab client, because Schwab fundamentals are only a base that the Router overlays with SEC EDGAR statement facts — caching in the client would persist the pre-overlay blob (FCF=0) and re-trip the FCF hard-filter. Caching the *merged* result means a warm re-run skips both the Schwab and the EDGAR calls. Always check the cache before hitting the network.
- **Save raw for debugging.** Enforced as a code path, not just a discipline: `pkg/rawcapture` (the zero-import L0 hook) hooks the single chokepoint in each API client (`schwab.Client.executeRequest`, yfinance `executeYFinanceRequest`) and, on a 2xx, hands the body to an injected `rawstore.Sink` (L4) which archives the raw bytes flat to `data/raw/<source>__<endpoint>__<symbol>__<YYYYMMDD-HHMMSS>.json` before handing an unchanged, re-readable body back to the caller. Capture is **ON by default** (the surprising run is the one you didn't think to arm); opt out with `MYCASE_CAPTURE=0/off`. OAuth token/refresh responses use a separate path and are never captured. Growth is bounded by retention, not by declining to capture: `mycase raw prune` (and a best-effort pass at the end of every command) enforces two independent ceilings — age (`raw.retain_days`, default 14) and total size (`raw.max_size_mb`, default 512, oldest-first), both env/flag-overridable (`MYCASE_RAW_RETAIN_DAYS` / `MYCASE_RAW_MAX_SIZE_MB`). Triage the archive schema-blind with `mycase raw ls` (filterable table), `raw show [query]` (pretty-print the newest match), and `raw path [query]` (print the path for `jless`/`jq`); for Schwab fundamentals specifically, `raw inspect [symbol]` reruns the production mapper and shows each raw wire field beside its mapped value (the tool that localizes bind-zero bugs like `pick 0/N`). Analyze the archived JSON offline rather than re-hitting the endpoint from different angles. Offline replay is implemented: set `MYCASE_REPLAY=1` and each client chokepoint serves the newest matching archived body as a synthetic 200, skipping the network, token fetch, and rate limiter entirely — so `pick`/`report`/etc. rerun the whole pipeline with zero live calls (`docs/03-roadmap.md` Phase 11).
...
Routing is centralized in `pkg/datafetcher/router.go`. `Router` holds `schwabClient *schwab.Client` and an optional `edgarSource` (nil ⇒ pre-EDGAR behavior); every method routes on `schwab.IsUSTicker(ticker) && r.schwabClient != nil` → Schwab, else Yahoo. `IsUSTicker`/`StripUSPrefix` (`pkg/broker/schwab/market.go`) recognize `US:`/`NASDAQ:`/`NYSE:` prefixes; `NSE:`/`BSE:` (and `.NS`/`.BO`) are not special-cased in the router and fall through to Yahoo, with prefix stripping done inside `pkg/yfinance`. `FetchQuotes`/`FetchFundamentals` split the ticker list and send the US leg through one shared ordered-chain helper (`usPrimaryWithYahooFallback`): **Schwab primary, Yahoo fallback on error** (original Schwab error returned if Yahoo also fails), and Yahoo directly when `schwabClient == nil`. Single-ticker historical methods route to Schwab with **no** fallback by design (a spliced multi-provider price series is worse than a clean, retryable failure).

**The consumer-defined `stockpicker.DataFetcher` is now split into capability interfaces** (`pkg/stockpicker/types.go`): `DataFetcher` composes `PriceSource` (`FetchHistoricalDataWithTimestamps`, `FetchHistoricalPrices`) and `FundamentalsSource` (`FetchFundamentals`), so a caller depends on only what it uses and `*datafetcher.Router` satisfies all of them (Phase 10d). `SectorSource` was deliberately **not** added: sector never flows through the router — it is backfilled from the constituents CSV by `InjectSectors` (Phase 10a, a GICS source), and otherwise rides on `Fundamentals.Sector` — so a router `SectorSource` would be dead surface.
...
| Data sources | 🟡 Partial | Router prefix-routing with logged Schwab→Yahoo fallback exists (`datafetcher/router.go`), consolidated into one ordered-chain helper (`usPrimaryWithYahooFallback`), but **~7 bypass paths** still hit yfinance directly (server, benchmark leg, calibrate/monitor/report/backtest, run.go fallbacks + EBM io.go). Provenance is recorded: `prices`/`fundamentals`/`selections` carry a `source` column, and `marketdata.Fundamentals` carries record-level `Source` + per-field `FieldSources` (with EDGAR filing detail) surfaced in `pipeline show`/reports (Phase 10d). `DataFetcher` is now split into `PriceSource`/`FundamentalsSource` capability interfaces (sector comes from the CSV, not the router). |
...
The remaining sections (§3 config, §5 algorithms, §6 pipelines, §7 testing, §8 merge) are **judgment rubrics**, not line-level rules — they are better kept in this doc and consulted deliberately during design/review than injected into every prompt. Recommendation: **do not promote §3/§5–§8 to steering**; instead cross-link this doc from `docs/04-architecture.md` as the design-review rubric, and keep the three existing steering files as the enforced subset. Revisit only if a specific principle starts being violated repeatedly in practice — that is the signal to codify it mechanically (as R16 did for layering).

...
- **Save raw for debugging.** Enforced as a code path, not just a discipline: `pkg/rawcapture` (env-gated by `MYCASE_CAPTURE`) hooks the single chokepoint in each API client (`schwab.Client.executeRequest`, yfinance `executeYFinanceRequest`) and, on a 2xx, archives the raw body flat to `data/raw/<source>__<endpoint>__<symbol>__<YYYYMMDD-HHMMSS>.json` before handing an unchanged, re-readable body back to the caller. Disabled by default (zero overhead); OAuth token/refresh responses use a separate path and are never captured. Analyze the archived JSON offline rather than re-hitting the endpoint from different angles. Offline replay is implemented: set `MYCASE_REPLAY=1` and each client chokepoint serves the newest matching archived body as a synthetic 200, skipping the network, token fetch, and rate limiter entirely — so `pick`/`report`/etc. rerun the whole pipeline with zero live calls (`docs/roadmap.md` Phase 11).

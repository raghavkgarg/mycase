...
func (c *Cache) StoreFundamentalsJSON(ctx context.Context, ticker string, data []byte) error {
	_, err := c.db.ExecContext(ctx, `
		INSERT INTO fundamentals (ticker, fetched_at, raw_json) VALUES (?, CURRENT_TIMESTAMP, ?)
		ON CONFLICT (ticker) DO UPDATE SET fetched_at = CURRENT_TIMESTAMP, raw_json = EXCLUDED.raw_json`,
		ticker, string(data),
	)
	return err
}
